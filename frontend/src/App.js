...
const IntellectualPropertyPage = lazyWithPreload(() => import('./components/IntellectualPropertyPage'));
const SecurityPolicyPage = lazyWithPreload(() => import('./components/SecurityPolicyPage'));
const AccessibilityStatementPage = lazyWithPreload(() => import('./components/AccessibilityStatementPage'));
const DomainManagementPage = lazyWithPreload(() => import('./components/DomainManagementPage'));
...
function VisitorHeartbeat() {
  ...
  useEffect(() => {
    ...
    const sendSiteView = async () => {
      if (cancelled || !storefrontSubdomain) return;
    ...
    const sendClick = async (e) => {
      ...
      const payload = JSON.stringify({
        ...
        productKey: e.target.closest('[data-product-id]')?.getAttribute('data-product-id') || ''
      });

      try {
        if (typeof navigator !== 'undefined' && typeof navigator.sendBeacon === 'function') {
          const blob = new Blob([payload], { type: 'application/json' });
          navigator.sendBeacon(`${API_BASE}/api/metrics/click`, blob);
          return;
        }
      } catch (err) { void err; }

      try {
        await fetch(`${API_BASE}/api/metrics/click`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: payload,
          keepalive: true
    ...
    <div className={`min-h-screen ${isStorefront ? '' : 'gradient-bg'} flex flex-col ${hideChrome ? '' : ''}`}>
      ...
      <div className="flex-1">
        <Routes>
          ...
          <Route path="/sites/:id/settings" element={<WithSuspense><ProtectedRoute><DesktopOnlyGate><SiteSettingsPage /></DesktopOnlyGate></ProtectedRoute></WithSuspense>} />
          <Route path="/sites/:id/settings/design" element={<WithSuspense><ProtectedRoute><DesktopOnlyGate><SiteSettingsPage forcedSection="design" /></DesktopOnlyGate></ProtectedRoute></WithSuspense>} />
          <Route path="/sites/:id/settings/general" element={<WithSuspense><ProtectedRoute><DesktopOnlyGate><SiteSettingsPage forcedSection="general" /></DesktopOnlyGate></ProtectedRoute></WithSuspense>} />
          ...
          <Route path="/intellectual-property" element={<WithSuspense><IntellectualPropertyPage /></WithSuspense>} />
          <Route path="/security-policy" element={<WithSuspense><SecurityPolicyPage /></WithSuspense>} />
          <Route path="/accessibility" element={<WithSuspense><AccessibilityStatementPage /></WithSuspense>} />
...
function isPlatformHost(hostname) {
  const h = (hostname || '').toString().trim().toLowerCase().replace(/\.$/, '');
  if (!h) return true;
  if (h === 'localhost' || h === '127.0.0.1') return true;
  // Match both odelink.shop and www.odelink.shop with absolute certainty
  if (h === 'odelink.shop' || h === 'www.odelink.shop') return true;
  // Subdomain check for odelink.shop
  if (h.endsWith('.odelink.shop')) return true;
  return false;
}
