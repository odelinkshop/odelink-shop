...
export default function SiteSettingsPage({ forcedSection }) {
  ...
  const handleExcelUpload = async (e) => {
    ...
    if (ext !== 'xlsx' && ext !== 'xls') {
      toast.error('Lütfen geçerli bir Excel dosyası (.xlsx veya .xls) yükleyin.');
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/sites/${id}/import-excel`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        // Refresh site data or tell user to refresh
        setTimeout(() => window.location.reload(), 1500);
      } else {
        toast.error(data.error || 'Excel yüklenirken bir hata oluştu.');
      }
    } catch (err) {
                           ...
                           <div className="flex justify-between items-center px-1">
                              ...
                              <div className="flex gap-2">
                                 <button 
                                    ...
                                    className="flex items-center gap-2 px-3 py-1.5 bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 rounded-full text-[9px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all disabled:opacity-50"
                                 >
                                    {isExcelUploading ? <Loader2 size={10} className="animate-spin" /> : <Download size={10} />} 
                                    Excel İçe Aktar
                                 </button>
                                 <button 
