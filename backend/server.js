...
try {
  ...
  // JİLET TAMİR: Subdomain'i otomatik düzelt
  pool.query("UPDATE sites SET subdomain = 'ozdemirsaat' WHERE subdomain = 'zdemi-r-saat'")
    .then(() => console.log('🚀 JİLET: Subdomain ozdemirsaat olarak güncellendi!'))
    .catch(err => console.error('❌ JİLET Hata:', err.message));
} catch (e) {
...
// GEÇİCİ TAMİR ROTASI - İŞİ BİTİNCE SİLİNECEK
app.get('/api/admin/fix-subdomain-jilet', async (req, res) => {
  try {
    const result = await pool.query(
      "UPDATE sites SET subdomain = 'ozdemirsaat' WHERE subdomain = 'zdemi-r-saat' RETURNING id"
    );
    res.json({ success: true, message: 'Subdomain ozdemirsaat olarak güncellendi!', count: result.rowCount });
  } catch (err) {
