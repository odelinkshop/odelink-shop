/**
 * GENT Theme Service - GERÇEK ÇÖZÜM
...
class GentThemeService {
  constructor() {
    this.themeBasePath = path.join(__dirname, '../themes/gent.framer.website');
    this.themeHtmlPath = path.join(this.themeBasePath, 'wear.framer.website/index.html');
  }

  /**
   * GENT temasını render et
   */
...
<script>
  ...
  style.textContent = `
    :root {
      --framer-color-accent: #1a1a1a !important;
      --framer-color-primary: #1a1a1a !important;
    }
    /* Profesyonel bir görünüm için bazı renk değişimleri */
    .framer-1a1a1a { background-color: #1a1a1a !important; }
    /* Butonları daha keskin yapalım */
    button, .framer-button { border-radius: 4px !important; }
  `;
...
<script>
  ...
  // GENT temasının initialization'ını bekle ve override et
  var initAttempts = 0;
  ...
  console.log('✅ Odelink: Product injection ready');
      ...
      html = html.replace(/<title>.*?<\/title>/i, `<title>${siteSettings.name || 'GENT'} - Premium Men's Fashion</title>`);
      html = html.replace(/<meta property="og:title" content="Wear">/i, `<meta property="og:title" content="${siteSettings.name || 'GENT'}">`);
      html = html.replace(/<meta name="twitter:title" content="Wear">/i, `<meta name="twitter:title" content="${siteSettings.name || 'GENT'}">`);
      
      const description = siteSettings.description || 'Discover our premium men’s fashion collection—high-quality, stylish apparel for modern lifestyles.';
      ...
      console.log(`✅ GENT teması hazır: ${shopifyProducts.length} ürün`);
      return html;
      
    } catch (error) {
      console.error('❌ GENT theme error:', error);
      throw error;
    }
  }
  ...
  convertToShopifyFormat(products) {
    ...
    return products.map((p, index) => {
      ...
      return {
        ...
        // Görseller - GENT teması bunu kullanıyor
        images: {
...
module.exports = new GentThemeService();
