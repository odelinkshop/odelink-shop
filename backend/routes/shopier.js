...
const { normalizeShopierUrl, fetchAllProducts, fetchShopierCatalog, requestWithRetry } = require('../services/shopierCatalogService');
...
router.post('/scrape', async (req, res) => {
  try {
    ...
    // Use fetchShopierCatalog which handles caching and full shop data
    const catalog = await fetchShopierCatalog(normalized, { 
      skipDetails: true,
      debug: true 
    });
    ...
    return res.json({
      success: true,
      data: {
        storeName: catalog.shopName || 'Shopier',
        products: catalog.products || [],
        categories: catalog.categories || [],
        totalProducts: catalog.totalProducts || 0,
        shopierUrl: catalog.shopierUrl || normalized
      }
    });
    ...
    // Check if it's a known user-side error (400) or a timeout/fetch error
    const looksLikeBadInput = message.toLowerCase().includes('geçersiz') || 
                             message.toLowerCase().includes('gecersiz') ||
                             message.toLowerCase().includes('bulunamadı') ||
                             message.toLowerCase().includes('yavaş') ||
                             status === 400;

    if (looksLikeBadInput) {
      return res.status(400).json({
        success: false,
    ...
    // Default to 500 but with the actual message if possible
