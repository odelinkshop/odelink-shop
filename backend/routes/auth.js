...
// Enhanced Registration Guard (v4.0)
const registerSchema = Joi.object({
  name: Joi.string()
    .trim()
    .min(3)
    .max(50)
    .pattern(/^[a-zA-ZğüşıöçĞÜŞİÖÇ\s]+$/) // Only letters and spaces
    .custom((value, helpers) => {
      const parts = value.trim().split(/\s+/);
      if (parts.length < 2) return helpers.message('Lütfen adınızı ve soyadınızı tam girin.');
      // Block repetitive random strings like "asdfgh" or "aaaaa"
      if (/(\w)\1{3,}/.test(value.toLowerCase())) return helpers.message('Lütfen gerçek bir isim girin.');
      return value;
    })
    .required(),
  ...
  password: Joi.string()
    ...
    .message('Şifre en az 8 karakter olmalı; bir büyük harf, bir rakam ve bir özel karakter içermelidir.')
    .required(),
...
router.post('/register', async (req, res) => {
  try {
    ...
    // Domain Whitelist Validation - Sadece elit e-posta sağlayıcıları kabul edilir
    const emailDomain = normalizedEmail.split('@')[1];
    if (!ALLOWED_EMAIL_DOMAINS.includes(emailDomain)) {
      return res.status(400).json({ 
        error: `Sadece global e-posta servisleri (${ALLOWED_EMAIL_DOMAINS.join(', ')}) ile kayıt olabilirsiniz. Lütfen saygın bir e-posta adresi kullanın.` 
      });
    }
...
router.post('/google/callback', async (req, res) => {
  try {
    ...
    // Domain Whitelist Validation - Sadece elit e-posta sağlayıcıları kabul edilir
    const googleEmailDomain = googleIdentity.email.split('@')[1];
    if (!ALLOWED_EMAIL_DOMAINS.includes(googleEmailDomain)) {
      console.log('❌ Non-whitelist email rejected:', googleIdentity.email);
      return res.status(400).json({ 
        error: `Sadece global e-posta servisleri (${ALLOWED_EMAIL_DOMAINS.join(', ')}) ile giriş yapabilirsiniz. Lütfen saygın bir e-posta adresi kullanın.` 
      });
    }
