...
class Site {
  static async create(siteData) {
    const { userId, name, shopierUrl, customDomain, settings } = siteData;
    ...
    const query = `
      INSERT INTO sites (user_id, name, shopier_url, subdomain, custom_domain, settings, status, created_at, updated_at)
      VALUES ($1, $2, $3, $4, $5, $6, 'active', NOW(), NOW())
      RETURNING *
    `;
    
    try {
      const result = await pool.query(query, [userId, name, shopierUrl, subdomain, customDomain, settings || null]);
